...
/**
 * Main class for PermissionsCanyon.
 */
public class PermissionsPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (!new File(getDataFolder(), "config.yml").exists()) {
            ...
            getServer().getLogger().info("[PermissionsCanyon] Generating default configuration");
    ...
    protected void debug(String message) {
        if (getConfiguration().getBoolean("debug", false)) {
            getServer().getLogger().info("[PermissionsCanyon] [Debug] " + message);
        }
    }
    ...
    private void calculateAttachment(Player player) {
        ...
        for (Map.Entry<String, Object> entry : calculatePlayerPermissions(player.getName().toLowerCase(), lastWorld.get(player.getName())).entrySet()) {
            if (entry.getValue() != null && entry.getValue() instanceof Boolean) {
                ...
                getServer().getLogger().warning("[PermissionsCanyon] Node " + entry.getKey() + " for player " + player.getName() + " is non-Boolean");
            }
        }
    ...
    private void writeDefaultConfiguration() {
        ...
        getConfiguration().setHeader(
            "# PermissionsCanyon configuration file",
            ...
            "# for example, PermissionsCanyon has 'permissions.*', which automatically",
            ...
            "# will be displayed to the player if PermissionsCanyon prevents them from",
